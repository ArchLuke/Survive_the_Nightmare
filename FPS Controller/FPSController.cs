...
public class FPSController : MonoBehaviour 
{
	...
	[SerializeField] Animator _handGunAnimator = null;
	[SerializeField] Animator _rifleAnimator = null;
	[SerializeField] Animator _knifeAnimator = null;
	...
	private int _movementParameter = 0;

	// Timers
	private float 		_fallingTimer 					= 0.0f;

	private CharacterController _characterController	= null;
	[SerializeField]private PlayerMoveStatus	_movementStatus 		=	PlayerMoveStatus.NotMoving;
	...
	protected void Update()
	{
		...
		if (Input.GetButtonDown ("Crouch")) 
		{
			...
			_characterController.height = _isCrouching==true?_controllerHeight / 2.5f: _controllerHeight;
		}
		...
		{
			_movementStatus = PlayerMoveStatus.NotMoving;
			_handGunAnimator.SetInteger(_movementParameter,0);
			_rifleAnimator.SetInteger(_movementParameter,0);
			_knifeAnimator.SetInteger(_movementParameter,0);
			if (_isCrouching)
			{
		...
		else if (_isWalking)
		{
			...
			_handGunAnimator.SetInteger(_movementParameter,1);
			_rifleAnimator.SetInteger(_movementParameter,1);
			_knifeAnimator.SetInteger(_movementParameter,1);

		}
		else
		{
			_movementStatus = PlayerMoveStatus.Running;
			_handGunAnimator.SetInteger(_movementParameter,2);
			_rifleAnimator.SetInteger(_movementParameter,2);
			_knifeAnimator.SetInteger(_movementParameter,2);
