...
public class AIZombieStateMachine : AIStateMachine
{
	...
	public AIBoneControlType boneControlType
	{
		get { return _boneControlType; }
	}
	public float			speed    	
	{ 
		get{ return _speed;		}
	...
	// --------------------------------------------------------------------
	public override void TakeDamage( Vector3 position, Vector3 normal, Vector3 force, int damage, Rigidbody bodyPart, CharacterManager characterManager, int hitDirection=0, bool knife=false )
	{
		
		if (_BloodFX != null)
		{
			
			float angle = Mathf.Atan2(normal.x, normal.z) * Mathf.Rad2Deg + 180;

			if (effectIdx == _BloodFX.Length) effectIdx = 0;

			var settings=Instantiate(_BloodFX[effectIdx], position, Quaternion.Euler(0, angle + 90, 0));
			settings.GetComponent<BFX_BloodSettings>().GroundHeight = transform.position.y;
			effectIdx++;
		...
		if (_boneControlType!=AIBoneControlType.Animated || isCrawling || IsLayerActive("Cinematic") || attackerLocPos.z<0 || _health<1) shouldRagdoll=true;
